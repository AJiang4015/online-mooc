...
当你通过本地代码直接连接虚拟机 Docker 环境启动项目时，必须确保以下服务与端口正常开放：

1. **Nacos 端口**：Nacos 2.x 客户端不仅需要暴露 `8848`（HTTP 端口），还**必须暴露 `9848` 和 `9849`（gRPC 端口）**。若未暴露 gRPC 端口，本地服务在启动时将报找不到配置文件的错误。
2. **Elasticsearch 端口**：向量数据库（AIGC与搜索）配置连接的 ES 端口为 `19200`，请确保虚拟机存在对应容器，或修改配置文件与你当前虚拟机的 `9200` 端口对齐。
3. **Qdrant 与 MongoDB**：AIGC 模块分别需要连接向量库 Qdrant（端口 `6334`）以及 MongoDB（端口 `27017`），若未启动会导致相关服务不可用。
4. **其他基础中间件**：Redis（`6379`）、MySQL（`3306`）、RabbitMQ（`5672`）、XXL-JOB（`8880`）等需确保能够正常连接。

本地配置文件（`application-local.yml`）中的地址可通过环境变量覆盖，默认连接虚拟机：
- `TJ_NACOS_ADDR` 默认：`192.168.127.101:8848`
- `TJ_HOST` 默认：`127.0.0.1` （微服务注册到 Nacos 时的本机 IP，若是局域网调用可改为当前电脑局域网 IP）
- `TJ_ES_URI` 默认：`http://192.168.127.101:9200`
