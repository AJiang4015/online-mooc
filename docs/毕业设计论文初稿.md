...
结合系统业务场景，可以将平台功能划分为用户端功能、管理端功能和平台支撑功能三大部分。系统总体功能结构如图所示。
...
从学生用户角度出发，系统重点实现以下功能：

1. 课程浏览与搜索功能。用户能够在首页查看推荐课程、按照分类筛选课程，并通过关键词检索目标课程。
2. 下单与支付功能。用户能够将课程加入购物车、在结算页使用优惠券并完成在线支付。
3. 学习与考试功能。购课成功后，用户可进入我的课程页面，查看课表、设置学习计划、记录学习进度并参与章节练习或考试。
4. 问答与笔记功能。用户在学习过程中可以发起提问、发表评论、记录笔记并接收消息提醒。
5. 直播与 AI 助学功能。平台支持直播课程互动，并扩展提供 AI 智能问答和知识库检索能力。

从后台管理角度出发，系统重点实现以下功能：

1. 课程与分类管理。管理员能够维护课程分类、课程信息、课程目录和课程上下架状态。
2. 营销与订单管理。管理员能够配置优惠券规则、查看订单信息并处理退款申请。
3. 审核与数据统计。管理员能够审核互动内容、查看课程与交易数据看板，并辅助平台运营决策。

由于毕业设计论文篇幅有限，后续系统实现章节只选取课程搜索、优惠券下单、支付购课、学习记录和问答互动等重要功能进行重点说明，不对全部功能逐一展开。
...
系统数据库设计遵循“围绕核心业务、结构清晰、便于扩展”的原则，主要针对用户、课程、订单、支付、学习、互动和营销等核心数据进行建模。数据库采用 MySQL 存储结构化业务数据，同时结合 Redis、Elasticsearch、MongoDB 与 Qdrant 完成缓存、搜索和 AI 相关数据支撑。

系统数据库的核心实体包括用户、用户详情、角色、课程分类、课程、课程目录、订单、订单明细、支付订单、优惠券、用户优惠券、学习课表、学习记录、互动问题、互动回复和考试记录等。各实体之间的主要关系如图所示。
...
为了便于在论文中对数据库结构进行整体说明，本文进一步将系统中的重要数据表整理如表 3-1 所示。

表 3-1 重要数据表一览表

| 表名 | 所属模块 | 主要作用 | 关键字段 |
| --- | --- | --- | --- |
| `user` | 用户中心 | 存储平台登录账号与用户身份基础信息 | `id`、`username`、`cell_phone`、`password`、`type`、`status` |
| `user_detail` | 用户中心 | 存储用户扩展资料、头像、角色与学习相关统计信息 | `id`、`name`、`icon`、`role_id`、`course_amount` |
| `category` | 课程中心 | 维护课程分类层级结构，支撑课程归类和检索 | `id`、`name`、`parent_id`、`level`、`status` |
| `course` | 课程中心 | 存储课程主信息，是课程展示、购买和学习的核心主表 | `id`、`name`、`course_type`、`price`、`status`、`valid_duration` |
| `course_catalogue` | 课程中心 | 存储课程章节和小节目录，支撑课程播放与章节学习 | `id`、`course_id`、`type`、`parent_catalogue_id`、`media_id` |
| `course_content` | 课程中心 | 存储课程介绍、课程详情和适用人群等大文本信息 | `id`、`course_introduce`、`use_people`、`course_detail` |
| `order` | 交易中心 | 存储订单主信息，贯穿待支付、已支付、完成、关闭等状态流转 | `id`、`user_id`、`pay_order_no`、`status`、`total_amount`、`real_amount` |
| `order_detail` | 交易中心 | 存储订单中的课程明细，支撑课程发放、有效期控制与退款 | `id`、`order_id`、`course_id`、`real_pay_amount`、`status`、`refund_status` |
| `pay_order` | 支付中心 | 存储支付流水单据，记录支付渠道、金额与支付状态 | `pay_order_no`、`biz_order_no`、`pay_channel_code`、`amount`、`status` |
| `coupon` | 营销中心 | 定义优惠券模板及优惠规则，如满减、折扣、门槛与库存 | `id`、`name`、`discount_type`、`discount_value`、`threshold_amount`、`total_num` |
| `user_coupon` | 营销中心 | 记录用户实际领取到的优惠券实例，是结算核销的直接依据 | `id`、`user_id`、`coupon_id`、`term_begin_time`、`term_end_time`、`status` |
| `learning_lesson` | 学习中心 | 表示用户已购买课程形成的课表数据，记录学习状态与学习计划 | `id`、`user_id`、`course_id`、`status`、`week_freq`、`learned_sections` |
| `learning_record` | 学习中心 | 记录用户在每个课程小节上的学习进度与完成情况 | `id`、`lesson_id`、`section_id`、`user_id`、`moment`、`finished` |
| `interaction_question` | 学习互动 | 存储学习过程中的提问主帖，支撑问答交流主线 | `id`、`title`、`course_id`、`section_id`、`user_id`、`answer_times` |
| `interaction_reply` | 学习互动 | 存储问题回复与评论信息，形成问答互动链路 | `id`、`question_id`、`user_id`、`content`、`liked_times` |

在主要数据表设计上，`user` 与 `user_detail` 表负责用户基本信息与扩展资料；`course`、`course_content` 与 `course_catalogue` 表共同描述课程主体、课程详情和章节结构；`order`、`order_detail` 与 `pay_order` 表共同描述交易与支付过程；`learning_lesson` 与 `learning_record` 表用于描述用户的购课结果与学习进度；`coupon` 与 `user_coupon` 表用于支持优惠券营销与结算优惠；`interaction_question` 与 `interaction_reply` 表用于支撑学习问答社区。
